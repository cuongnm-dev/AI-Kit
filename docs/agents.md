...
> 🎯 **Runtime đơn target**: từ 2026-05-25, mọi agent chạy trong **ai-engine** (OpenCode fork). Claude Code / Cursor đã retire khỏi deploy target.

---

## 1. Bức tranh tổng thể

**Tổng ~53 agents** (ai-engine namespace, hợp nhất từ 22 Claude + 31 Cursor cũ), chia thành 4 nhóm:

- **9 stage agents** (`sdlc-*` — chạy trong pipeline `/resume-feature`) — có variant `*-pro` cho high-risk
- **11 tdoc agents** (`tdoc-*` — code → tài liệu) — từ `/from-code` đến `/generate-docs`
...
### 3.1 Stage agents (`sdlc-*` — chạy trong pipeline `/resume-feature`)
...
| `tdoc-exporter` | `/from-code` Phase 4 | Office files qua ai-mcp MCP |
...
## 5. Chi tiết stage agents — SDLC pipeline (ai-engine)
...
**Vai trò**: Test 1 feature, **produce 3 atomic artifacts** (I-001.16):

1. `test-evidence/{id}.json` — TC prose với `execution.status` set
2. `playwright/{id}.spec.ts` — executable spec, re-runnable bởi CI
3. `screenshots/{id}-step-NN-{state}.png` — captured DURING Playwright run (G-004 naming)
...
~21 agents (9 doc-* + 12 tdoc-* trong ai-engine namespace). Render canonical intel → Office output.
...
**tdoc-\* family** — chạy trong `/from-code` + `/generate-docs`:
...
| `tdoc-exporter` | Phase 4: upload + render Office qua ai-mcp MCP |

**doc-\* family** — chạy trong `/generate-docs` 3-mode pipeline:
...
**Khác biệt doc-\* vs tdoc-\***:
- `doc-*`: input từ tài liệu (docx/pdf/ảnh) qua `doc-intel` OCR — generate-docs 3-mode
- `tdoc-*`: input từ codebase qua `tdoc-researcher` — `/from-code` pipeline
