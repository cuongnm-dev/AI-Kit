...
- Docker đang chạy (cho MCP `ai-mcp` warm-start KB cache, optional)
...
- Min TC count theo I-001.15: `min_tc = max(5, AC×2 + roles×2 + dialogs×2 + errors + 3)`
...
Print next-step: **`/resume-module {parent-module-id}`** (entry mặc định post-ADR-006) lặp cho mỗi module trong handoff list — SDLC pipeline nhận bàn giao + advance pipeline. Dùng `/resume-feature {F-NNN}` chỉ khi muốn dispatch RIÊNG 1 feature.
...
> Bước tiếp: /resume-module M-001 (trong Cursor — entry mặc định)

# 5. SDLC từng module (Cursor) — entry mặc định post-ADR-006
/resume-module M-001    → ba → sa → tech-lead → dev-wave-1 → qa-wave-1 → reviewer → done
/resume-module M-002    (lặp cho mỗi module trong handoff list)
...
`docs/intel/` là single-source-of-truth giữa 3 entry-points (I-001):
...
- [ ] `docs/_idea/{idea-brief, impact-map, event-storming, story-map, pre-mortem, dedup-report, idea-graveyard, coherence-log, assumptions}.md` (workshop artifacts)
- [ ] `docs/modules/M-NNN-{slug}/_state.md` + `module-brief.md` (per module, post-ADR-006)
- [ ] `docs/modules/M-NNN-{slug}/_features/F-NNN-{slug}/feature-brief.md` (per feature, nested)
- [ ] `docs/intel/{module-map.yaml, feature-map.yaml, catalog.json}` updated

Sẵn sàng cho Cursor **`/resume-module M-NNN`** (entry mặc định post-ADR-006) lặp cho mỗi module trong handoff list.
...
Output là intake-stage intel chất lượng cao để SDLC pipeline nhận bàn giao mượt mà.
