...
### Step 2 — QA atomic triple gate (I-001.16)
...
```bash
ai-kit sdlc state update --op field --kind module --id M-NNN --field meta.last_producer --value close-feature
```
...
Skip Step 2 (QA triple). Logged + visible trong feature-map.yaml + feature-catalog.qa_status. Reviewer audit lần feature sau sẽ surface bypassed features (I-001 retrospective).
