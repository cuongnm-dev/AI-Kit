...
- **Output bước 1**: `docs/intel/{actor-registry, catalog, sitemap}.json` + `docs/modules/M-NNN-{slug}/_state.md` + `_features/F-NNN-{slug}/feature-brief.md`
- **Bước tiếp** (Cursor): **`/resume-module M-NNN`** (entry mặc định post-ADR-006 — dispatch full module pipeline) lặp cho mỗi module trong handoff list → `/generate-docs` sinh Office files. Dùng `/resume-feature F-NNN` chỉ khi muốn dispatch RIÊNG 1 feature.
...
- **Output bước 1**: 9 intel artifacts (system-inventory, code-brief, arch-brief, actor-registry, permission-matrix, sitemap, catalog, data-model, integrations) + `docs/modules/M-NNN-{slug}/` + `_features/F-NNN-{slug}/feature-brief.md` (status: implemented)
- **Bước tiếp**: `/intel-fill` (T3 fields như NFR/ATTT/dự toán) → `/generate-docs`. Nếu cần gap-close: **`/resume-module M-NNN`** (Cursor) dispatch full pipeline cho module có gap.
- **Onboarding chi tiết**: `ai-kit doc on-board-sdlc`
...
- **Output bước 1**: 4 intel artifacts + workshop docs (PRFAQ, impact-map, event-storming, story-map, pre-mortem) + `docs/modules/M-NNN-{slug}/` + `_features/F-NNN-{slug}/feature-brief.md` (status: proposed, source-type: idea-brainstormed)
- **Bước tiếp**: **`/resume-module M-NNN`** (entry mặc định) — SDLC pipeline tiếp nhận identical với tình huống 1/2.
