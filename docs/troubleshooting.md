...
Anh đã sửa file trong `~/.ai-kit/` — mất khi `git pull`. Options:
...
```
Phổ biến:
- `Permission denied: /data` → Mac/Win Docker Desktop OK; Linux: `chmod 777 ~/.ai-kit/mcp/ai-mcp/data`
- Port 8001 đang dùng → sửa `.env`: `AI_MCP_PORT=8002`

### `no matching manifest for linux/arm64`
Image cũ chưa multi-arch. Maintainer rebuild với `release-mcp.ps1` (đã default buildx multi-arch). Tạm thời force amd64:
```bash
docker pull --platform linux/amd64 o0mrblack0o/ai-mcp:v3.0.0
```
...
### Skill không trigger
- Restart ai-engine sau `ai-kit update`
- Verify deployed: `ai-kit status` → engine agents count
- Check `~/.config/engine/engine.json` có ai-mcp URL

### Agent không thấy intel
- Verify `docs/intel/` ở project root
- Check `_meta.json.artifacts[file].stale` — nếu stale → `/intel-refresh`
- Verify path: agent đọc `docs/intel/` relative to project, không phải engine config dir
...
Dùng **artifacts-studio** → canonical store (monorepo `docs/adr/0009`). Lệnh `ai-kit publish` đã **retired**.
...
- Rule mới đã inject vào `ai-engine/AGENTS.md` § Tool Usage Discipline
- Nếu vẫn gặp: agent đang chạy với cache cũ → restart ai-engine session
...
**Nguyên nhân**: Main thread fallback `general-purpose` thay vì specialist (Explore/Plan/doc-writer/policy-researcher).

**Fix (từ 2026-05-02 — P4.1 Specialist-first dispatch)**:
- `ai-engine/AGENTS.md` § Tool Usage Discipline đã có rule "Specialist-first dispatch"
...
### "SDLC pipeline ba/sa thiếu context dù đã đọc feature-brief.md"
