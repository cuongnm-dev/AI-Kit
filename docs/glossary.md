...
**Agent** — Một LLM-driven worker có vai trò cố định, chạy trong context riêng. Mỗi stage (ba/sa/tech-lead/dev/fe-dev/qa/reviewer) là một agent trong SDLC pipeline. Doc-generation agents (`tdoc-*`, `doc-*`) chạy trong background.
→ reference/agents

**ai-engine** — Sole runtime/deploy target (2026-05-25). OpenCode fork; encrypted bundle; deploy path `~/.config/engine/` (hoặc studio sandbox). Thay thế mô hình 3-vendor (Claude Code + Cursor + ai-engine).
...
→ Schema: `ai-engine/schemas/intel/`
...
**feature-brief.md** — Per-feature artifact (`{features-root}/{feature-id}/feature-brief.md`) là **primary `feature-req` file** mà `resume-feature` Step 3.0 đọc. Trong `/from-idea` (v0.27+) được enrich self-contained: Vision Context + Domain Context + Risks + Critical Assumptions + Source Spirals pointers — SDLC stage agents không cần lazy-read `_idea/*.md`.
...
**generate-docs** — Skill assembly: gom canonical intel + test-evidence + screenshots → render Office files (TKKT/TKCS/HDSD/test-cases) qua ai-mcp MCP.
...
**`_idea/` folder** — Workshop artifacts directory ở **workspace level** (không per-feature) cho `/from-idea`: idea-brief, impact-map, event-storming, story-map, pre-mortem, dedup-report, idea-graveyard, coherence-log, assumptions. Path: `{features-root}/_idea/`. **SDLC pipeline mặc định không đọc** — đó là context fossil, đã được digest qua Phase 5 crystallize vào `feature-brief.md`. Stage agents có thể lazy-read qua "Source Spirals" pointers nếu cần deep rationale.

**`idea-brainstormed`** — Giá trị `source-type` trong `_state.md` của features sinh từ `/from-idea` (Luồng C). Phân biệt với `SRS`/`BRD`/`user-input` (from-doc) và `code-reverse-engineered` (from-code). S-002 (formerly CD-20) enum.
...
**LIFECYCLE contract** — Hợp đồng "production line" giữa các agents/skills. Mỗi agent có 8 trường: ROLE, READ-GATES, OWN-WRITE, ENRICH, FORBID, EXIT-GATES, FAILURE, TOKEN-BUDGET. Xem `ai-engine/schemas/intel/LIFECYCLE.md`.
...
Cả 3 đều output cùng schema I-001 → SDLC pipeline nhận bàn giao identical. (Lưu ý: thuật ngữ "Luồng A/B" cũng dùng ở context khác — top-level "🅰 SDLC vs 🅱 Tài liệu nhà nước". Tùy ngữ cảnh.)
...
**MCP (Model Context Protocol)** — Chuẩn cho LLM tool servers. Team chạy 1 MCP server duy nhất tên `ai-mcp` ở `localhost:8001`.
...
**`source-type`** — Field trong `_state.md` frontmatter (S-002, formerly CD-20) phân biệt origin của feature: `SRS`/`BRD`/`user-input` (from-doc), `code-reverse-engineered` (from-code), `idea-brainstormed` (from-idea).
