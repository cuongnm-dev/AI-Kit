...
### 2.3 Các skill thường dùng?

| Mục đích | Skill |
|---|---|
| Phân tích tài liệu lớn (SRS 200 trang) | `/from-doc` |
| Brainstorm có cấu trúc từ ý tưởng | `/from-idea` |
| Code module + features mới | `/resume-module M-NNN` — entry mặc định post-ADR-006 |
| Review code | agent reviewer |
| Sinh Office files nghiệm thu | `/generate-docs` |
| Reverse-engineer codebase | `/from-code` |
| Soạn Đề án CĐS / HSMT | `/new-strategic-document` hoặc `/new-document-workspace` |

Tất cả skills chạy trên **ai-engine** runtime.
...
```
~/.ai-kit/              # repo cloned at root (flat layout)
  ├─ .git/              # repo state
  ├─ bin/               # ai-kit launcher (in PATH)
  ├─ ai-engine/         # source — deploy → ~/.config/engine/
  └─ mcp/ai-mcp/  # docker-compose + .env

~/.config/engine/
  ├─ agent/             # deploy từ repo ai-engine/agent/
  └─ skills/            # deploy từ repo ai-engine/skills/
```

### 3.2 Tôi tự sửa file trong `~/.config/engine/agent/` được không?

**Không nên** — lần `ai-kit update` tiếp theo sẽ ghi đè. Nếu muốn customize:

1. Maintainer: chỉnh qua **artifacts-studio** (ADR-0009) — không sửa trực tiếp deploy path
2. Hoặc tạo PR vào monorepo nếu đụng CLI/SDLC code

### 3.3 `ai-kit update` báo "Local changes detected"?

Repo `~/.ai-kit/` có thay đổi chưa commit. Hai lựa chọn:
...
# Hoặc giữ thay đổi
git -C ~/.ai-kit stash push -u
ai-kit update
git -C ~/.ai-kit stash pop
```
...
`ai-kit status` hiển thị `MCP image`. Mặc định `:latest`. Maintainer tag cụ thể trong `mcp/ai-mcp/.env` của repo.
...
Sau Phase 6 handoff: **`/resume-module M-NNN`** (entry mặc định post-ADR-006) lặp cho mỗi module trong handoff list. SDLC pipeline nhận bàn giao + advance pipeline. Dùng `/resume-feature F-NNN` chỉ khi muốn dispatch RIÊNG 1 feature.
...
ID **bất biến** sau khi commit (I-001/S-002 renumber, 2026-05-25). 3 entry-points (`from-doc`/`from-code`/`from-idea`) đều dùng cùng convention.
...
Quy tắc P-002 trong `AGENTS.md` của ai-engine bundle.

### 6.3 `source-type` trong `_state.md` có những giá trị nào?

I-001/S-002 quy định 4 giá trị (mỗi entry-point 1 giá trị):
...
- Không re-fetch khi đã có (I-001.9 — Reuse-first mandate)
...
`feature-brief.md` được enrich (v0.27) với Vision Context — SDLC stage agents có đủ context, không cần lazy-read `_idea/*.md` → tiết kiệm token downstream.
...
GitHub issue ở https://github.com/cuongnm-dev/ai-kit. Kèm output `ai-kit doctor` + `ai-kit version`.
...
### 9.6 SDLC pipeline có đọc `_idea/` không?

**Mặc định KHÔNG**. `_idea/*.md` là workshop docs ở workspace level (1 folder shared, không per-feature). Stage agents (ba/sa/dev/qa/reviewer) đọc:
...
### 11.2 Cấu trúc folder module post-ADR-006 (Aggregate v2)

```
docs/modules/M-001-iam/                           # MODULE — đơn vị pipeline (aggregate root)
├── _state.md                                     # state machine (current-stage, stages_queue, version OCC)
├── module-brief.md                               # 1 brief cho cả cluster nhiều feature
├── implementations.yaml                          # services[].path canonical (B-049)
├── ba/00-feature-spec.md                         # BA bao quát toàn module
├── sa/00-architecture-overview.md                # SA bao quát toàn module
├── designer/  ← chỉ tồn tại nếu screen_count > 0
├── security/  ← chỉ tồn tại nếu Path L hoặc có PII
├── tech-lead/wave-plan.yaml                      # phân rã task theo wave + feature (S-001 module-level)
├── reviewer/final-verdict.md                     # review cả module 1 lần
└── _features/                                    # underscore prefix sorts to top in IDE (R12)
    ├── F-001-citizen-account-pin-link/
    │   ├── feature-brief.md                      # FeatureSpec ≤200 lines (R1, renamed từ _feature.md)
    │   ├── implementations.yaml
    │   ├── ba/user-stories.md + acceptance-criteria.md
    │   ├── sa/design.md + api-contract.md
    │   ├── dev/implementation-notes.md
    │   └── qa/test-plan.md (test-evidence canonical ở docs/intel/test-evidence/F-001.json)
    ├── F-002-.../
    └── F-003-.../
```

**Lưu ý (STRICT-MODE 2026-05-08)**: `_features/` (R12 underscore prefix), `feature-brief.md` (R1 canonical — alias period terminated; legacy `_feature.md` flagged HIGH bởi `verify --scopes legacy_residue` rule `pre-r1-feature-filename`, phải migrate), `catalog.json` R7 merged nested (legacy split `feature-catalog.json` + `module-catalog.json` cũng flagged HIGH, phải migrate), test-evidence canonical at `docs/intel/test-evidence/{F-NNN}.json` (CD-10 #16, không stub trong feature folder per B-082). Workspace có legacy artifacts → chạy `ai-kit sdlc migrate-legacy --workspace .` (B-088) để consolidate trước khi pipeline advance được.
...
**Project legacy (pre-ADR-003)**: STRICT-MODE 2026-05-08 — `/resume-feature F-NNN` REJECTED với blocker LEGACY-WORKSPACE-001. Workspace phải migrate qua `ai-kit sdlc migrate-legacy --workspace .` (B-088) trước. Sau migration, dùng `/resume-module {assigned_M_id}` (canonical post-ADR-006).
...
- Skill chạy trên ai-engine runtime, dùng Opus 4.7 cho phỏng vấn/dedup chất lượng cao.
...
8. Sao lưu + reset state nguyên tử (I-002 bắt buộc next-action)
...
| F | Bổ sung thủ công sau /from-doc — không muốn bị ghi đè | ★★★ | I-001.3 `locked_fields[]` (mọi mode đều tôn trọng) |
...
### 13.6 Feature cross-cutting lan ra 5+ module — S-003 enforce thế nào?
...
→ S-003 enforcement đảm bảo tính toàn vẹn cross-cutting. Chi tiết: `ai-engine/AGENTS.md#S-003`.

### 13.7 Nghi ngờ LLM bịa nội dung trong intel — kiểm tra thế nào?

**Trích xuất có confidence** (I-001.13): mọi entry intel đều có `confidence: high|medium|low|manual` + `evidence[]` + `source_producers[]`.
...
Tính năng tương lai: `/from-doc --non-interactive` với defaults preset. Đang track tại backlog.
...
**I-001.13 confidence**: nếu tech-lead `low_confidence` về việc cô lập task → đánh dấu `[NEEDS-VALIDATION]`, PM chặn dev wave cho đến khi user xem lại.
...
- Đọc `ai-engine/schemas/intel/CHANGELOG.md` xem delta giữa các phiên bản
- Backup `cp -r docs/intel/ docs/intel.bak/`
- Sửa intel JSON theo schema mới (vd. S-002 thêm field `module_id` cho feature-catalog post-ADR-003)
...
I-001.1 yêu cầu producer phải validate trước khi ghi. Intel cũ có thể có:
...
> **Note (2026-05-25)**: Section mô tả Cursor Background Agent batch flow trước sole-target pivot. Cursor retired; dùng ai-engine equivalent. Giữ cho historical context.
