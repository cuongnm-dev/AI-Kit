...
> - Pipeline SDLC có những thành phần gì (ai-engine / MCP)
...
- ai-engine agents (ba/sa/dev/qa) làm SDLC pipeline
...
3 step:
1. **INGESTION** (ai-engine) — biến input (tài liệu hoặc code) thành intel layer
2. **SDLC** (ai-engine) — phát triển feature qua các stage có quy trình
3. **RENDERING** (ai-engine + MCP) — biến intel thành Office files
...
```
   ...
   └── ...
            ...
            │
            ▼   (per MODULE, on ai-engine — post-ADR-006 default)

   /resume-module M-001
            │
            ▼
   ba → sa → tech-lead → dev-wave-1 → qa-wave-1 → reviewer
   (PM dispatch agents per stages_queue, cover module-level + feature-level work
    cho mọi feature trong module — atomic loop tới done hoặc blocked)
            │
            ▼
   /close-feature F-001  (lặp cho mỗi feature trong module sau reviewer-Pass)
            │
...
```
   ...
   /resume-module M-001
            │
            ▼
   ba → sa → tech-lead → dev-wave-N → qa-wave-N → reviewer → close-feature lặp
            │
            ▼   (after all features done)
...
## 5. SDLC pipeline — Bên trong ai-engine

Khi `/resume-module M-NNN` (entry mặc định post-ADR-006), Cursor dispatch agents tuần tự theo `stages_queue` trong `docs/modules/M-NNN-{slug}/_state.md`:

> **Khi nào dùng `/resume-feature F-NNN`**: legacy single-feature mode — chỉ khi muốn dispatch riêng 1 feature trong module có sẵn (skip pipeline các feature khác cùng module). 90% post-handoff dùng `/resume-module` đúng.
...
│   ► /new-module, /new-feature, /resume-module (default), /close-feature │
│   ► /resume-feature chỉ dùng khi single-feature dispatch (legacy)   │
...
**Một module đầy đủ thường đi qua cả 2 tool:**
1. Claude `/from-doc` (or `/from-code`, `/from-idea`) → khởi tạo intel + scaffold modules + features
2. Cursor `/resume-module M-NNN` cho TỪNG module → SDLC stages full pipeline (post-ADR-006 entry mặc định). Dùng `/resume-feature F-NNN` chỉ khi cần single-feature dispatch.
...
- `lifecycle` — Contract đầy đủ (I-001.21)
