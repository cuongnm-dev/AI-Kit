...
**Quyết định**: Build qua `docker buildx build --platform linux/amd64,linux/arm64 --push`. Image public ở `o0mrblack0o/ai-mcp`. Maintainer chạy `release-mcp.{ps1,sh}` để rollout.
...
→ Tham khảo `ai-engine/schemas/intel/LIFECYCLE.md` (bản canonical).
...
## ADR-007: English-only cho agent/skill prompts (P-002)
...
**Ngày**: 2026-04-28 · **Trạng thái**: ✅ Adopted (I-001.16)
...
4. **Read-before-Edit rule**: Inject vào `ai-engine/AGENTS.md` § Tool Usage Discipline + 2 skills (`code-change`, `hotfix`). Mục tiêu giảm 24% workflow errors.
5. **Specialist-first dispatch**: Rule mới trong `ai-engine/AGENTS.md` nudge main thread chọn specialist agent (Explore/Plan/doc-writer/tdoc-*) trước khi fallback `general-purpose`. Mục tiêu giảm $230 (98 dispatches) `general-purpose` cost.
...
Plan + execution log: nằm trong maintainer governance hub (private meta-repo, không ship trong ai-kit dist).
...
4. **Spiral 4 User Story Mapping** (Patton) — backbone → MVP cut + TC seeds (I-001.14-15)
5. **Phase 4.5 Pre-mortem** — failure projection + success pathway + risk register propagation

Output: 4 intel artifacts (`actor-registry`, `permission-matrix`, `sitemap`, `feature-catalog`) + per-feature `_state.md` (I-001/S-002 renumber, source-type: `idea-brainstormed`) + `feature-brief.md` + test-evidence seeds. Tag producer: `manual-interview` (existing enum, no schema bump).
...
- ✅ Output đồng nhất với `/from-doc` + `/from-code` (cùng schema I-001) — SDLC pipeline nhận bàn giao identical
...
**Implementation**: 17 files dưới `ai-engine/skills/from-idea/` (1 SKILL.md, 1 INTEL_INTEGRATION.md, 8 phases/, 7 notepads/). Cập nhật AGENTS.md thêm `from-idea` producer + `idea-brainstormed` source-type enum value.
...
## 2026-05-25 — ai-engine sole-target pivot
- **Status**: Accepted (2026-05-25)
- **Context**: 3-vendor strategy (Claude Code + Cursor + ai-engine) caused parity overhead, duplicated bundles, and inconsistent rule scheme (CD-NN flat).
- **Decision**: ai-engine becomes the sole runtime/deploy target. Claude Code + Cursor retired as deploy paths (still usable as IDE editors for source). CD-NN renumbered to P/T/I/S/G scheme. Cross-vendor parity gate retired.
- **Consequences**: Simplified bundle (encrypted artifact only), unified rule namespace, bootstrap legacy cleanup flow added.
