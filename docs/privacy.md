...
Telemetry mặc định **BẬT** cho thành viên team (cuongnm-dev/ai-kit). Bạn có thể tắt bất cứ lúc nào:
...
| `by_model` | `{ "claude-sonnet-4-6": 145, "ETC AI": 23 }` | Model usage |
...
## Cách dữ liệu di chuyển — local + manual transfer
...
> **Không có cloud sync, không có auto-upload.** ai-kit chỉ ghi share JSON xuống đĩa local; mọi lần truyền đi đều do bạn chủ động.
...
- **Audit**: file `~/.ai-kit/.telemetry-config.json` ghi nhận `last_export` timestamp
...
- **v0.37.0** (2026-05-07): local export + manual transfer
- **2026-05-08**: huỷ kế hoạch cloud auto-sync — telemetry vĩnh viễn local-only
