...
> Bộ công cụ AI dùng chung cho team: **ai-engine agents/skills + ai-mcp MCP**, đóng gói qua `ai-kit` CLI.
...
`ai-kit` là **CLI helper runtime** trên client. Product content (agents/skills) ship qua **encrypted artifact + studio canonical revision**; maintainer publish qua **artifacts-studio** (xem ADR-0008/0009 trong monorepo `docs/adr/`). Lệnh `ai-kit update` vẫn có thể dùng trên install legacy.
...
| Vai trò | Bắt đầu ở đâu | Skills quan trọng nhất |
|---|---|---|
| **PM/Maintainer** | maintainer | artifacts-studio publish, MCP release (`packages/ai-mcp`) |
...
**Sinh tài liệu (ai-engine skills):**
...
```
  ...
  (ai-engine)        (SDLC pipeline)    (ai-engine → MCP)
        │                 │                 │
        ▼                 ▼                 ▼
